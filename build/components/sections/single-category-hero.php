...
if (is_array($card_group)) {
  ...
  $czena_i_razmery = (!empty($card_group['czena_i_razmery']) && is_array($card_group['czena_i_razmery'])) ? $card_group['czena_i_razmery'] : [];
  $text_small = isset($card_group['text_small']) ? (string)$card_group['text_small'] : '';
  $warp = isset($card_group['warp']) ? (string)$card_group['warp'] : '';
}
?>
        ...
        <div class="container">
            ...
            <div class="single-category-hero__wrapper">
                        ...
                        <div class="single-category-hero__swiper-wrapper swiper-wrapper">
                          <?php if (!empty($card_imgs)) : ?>
                            <?php foreach ($card_imgs as $img_index => $card_img) :
                              if (!is_array($card_img)) continue;

                              $url = isset($card_img['url']) ? (string)$card_img['url'] : '';
                              if ($url === '') continue;

                              $alt = isset($card_img['alt']) ? (string)$card_img['alt'] : '';
                              $title = isset($card_img['title']) ? (string)$card_img['title'] : '';
                              ?>
                                  <div class="single-category-hero__slide swiper-slide" <?php echo (int)$img_index === 0 ? 'data-src-copy' : ''; ?>>
                                      <img
                          ...
                          <?php else : ?>
                              <div class="single-category-hero__slide swiper-slide" data-src-copy>
                                  <img
                                          src="<?php echo esc_url(get_template_directory_uri() . '/assets/images/single-category-hero/1.jpg'); ?>"
                                          alt="Изображение"
                                          title="Изображение"
                                          width="720"
                                          height="540"
                ...
                <div class="single-category-hero__item">
                  ...
                  <?php if (!empty($warp)) : ?>
                      <p class="single-category-hero__text" style="margin-bottom: 8px">Основа:</p>
                      <p style="color: #58687a"><?php echo esc_html($warp); ?></p>
                  <?php endif; ?>

                    <p class="single-category-hero__text">Выберите размер:</p>

                    <ul class="single-category-hero__size-inner" data-size-parent>
                      <?php foreach ($czena_i_razmery as $i => $size_item) :
                        if (!is_array($size_item)) continue;

                        $size = isset($size_item['size']) ? (string)$size_item['size'] : '';
                        if ($size === '') continue;
                        ?>
                          <li class="single-category-hero__size<?php echo ((int)$i === 0) ? ' active' : ''; ?>"
                              data-size
                              data-size-name="<?php echo esc_attr($size); ?>">
                    ...
                    <div class="single-category-hero__price-wrapper">
                        <div class="single-category-hero__price-inner">
                          <?php foreach ($czena_i_razmery as $i => $size_item) :
                            ...
                            $rawPrice = isset($size_item['price']) ? (string)$size_item['price'] : '';
                            $priceText = (trim($rawPrice) !== '') ? esc_html($rawPrice) : 'Цена уточняется';
                            ?>
                              <p class="single-category-hero__price<?php echo ((int)$i === 0) ? ' active' : ''; ?>"
                                 data-size-name="<?php echo esc_attr($size); ?>">
                                <?php echo $priceText; ?>
                              </p>
                          <?php endforeach; ?>

                            <p class="single-category-hero__price-text">
                                Актуальные цены на подходящие вам варианты узнавайте напрямую у менеджеров
                            </p>
                        </div>

                        <button class="single-category-hero__button" data-popup-request-open data-info-fill>
                            Запросить цены
                        </button>
                    </div>
                    ...
                    <a class="single-category-hero__link" href="#description">
                        Читать подробнее
